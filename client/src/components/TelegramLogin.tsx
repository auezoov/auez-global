      ...
      <div className="bg-black/40 backdrop-blur-xl rounded-3xl p-8 w-full max-w-md border border-white/10 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="flex justify-center mb-6">
            <button
              onClick={() => {
                // Trigger Telegram widget click
                const telegramButton = document.querySelector('#telegram-login-widget iframe') as HTMLIFrameElement
                if (telegramButton && telegramButton.contentDocument) {
                  const button = telegramButton.contentDocument.querySelector('button')
                  if (button) {
                    button.click()
                  }
                }
              }}
              className="group relative inline-flex items-center justify-center px-8 py-4 bg-gradient-to-r from-cyan-500 to-blue-500 text-white font-semibold rounded-2xl shadow-lg shadow-cyan-500/25 hover:shadow-cyan-500/40 transition-all duration-300 hover:scale-105 active:scale-95"
            >
          ...
          {/* Hidden Telegram Widget (for functionality) */}
          ...
          {isLoading && (
            <div className="text-center text-white">
              ...
              <p className="mt-2 text-sm text-gray-300">Аутентификация...</p>
            </div>
          )}
